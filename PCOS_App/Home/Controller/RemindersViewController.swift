...
class RemindersViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        // Use default cell styles instead of registering clear cells so standard UI works
    ...
    private func loadPreferences() {
        ...
        if let storedDates = defaults.array(forKey: ReminderPreferences.mealTimesKey) as? [Date] {
            ...
            // Defaults (Breakfast 8am, Lunch 1pm, Dinner 7pm)
            mealTimes = [
        ...
        // Adjust meal times array size to match frequency if it went out of sync
        adjustMealTimesArray()
        
        workoutEnabled = defaults.bool(forKey: ReminderPreferences.workoutEnabledKey)
        workoutTime = defaults.object(forKey: ReminderPreferences.workoutTimeKey) as? Date ?? makeDate(hour: 17, min: 30) // Default 5:30pm
        
        sleepEnabled = defaults.bool(forKey: ReminderPreferences.sleepEnabledKey)
        sleepTime = defaults.object(forKey: ReminderPreferences.sleepTimeKey) as? Date ?? makeDate(hour: 22, min: 0) // Default 10:00pm
    }
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if section == 0 {
            ...
            return 2 + mealFrequency // Toggle + Frequency Stepper + N Time cells
        }
        if section == 1 {
            return workoutEnabled ? 2 : 1 // Toggle + (Time cell if enabled)
        }
        if section == 2 {
            return sleepEnabled ? 2 : 1 // Toggle + (Time cell if enabled)
        }
        return 0
    }
